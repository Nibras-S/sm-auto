...
export default function CategoryCard({ category, index = 0, isListPage = false }) {
  ...
  // If we are on the main categories list page, render the unified premium tall portrait layouts
  if (isListPage && category.type) {
    const isTallWhite = category.type === "tall-white" || category.type === "small-white";
        ...
        {isTallWhite && (
          <div className="h-full w-full relative">
            <div className="hidden lg:block absolute lg:top-9 lg:left-9 z-20">
              {category.icon && (
                <div className="w-14 h-14 rounded-full bg-neutral-100/90 border border-neutral-200/30 flex items-center justify-center text-neutral-600 shadow-sm">
            ...
            <div className="absolute left-5 top-3 lg:left-9 lg:top-1/2 lg:-translate-y-1/2 z-20 max-w-[55%] pointer-events-none">
              ...
              <p className="text-neutral-500 text-xs lg:text-[13.5px] font-semibold mt-2 leading-relaxed">
                {category.tagline}
              </p>
            </div>
            ...
            <img
              src={category.image}
              alt={category.name}
              className={category.imageClass || "absolute right-[-2%] bottom-[12%] lg:top-1/2 lg:-translate-y-1/2 lg:bottom-auto lg:right-4 w-[62%] sm:w-[50%] lg:w-[65%] max-w-none object-contain transition-all duration-700 ease-out group-hover:scale-105 group-hover:translate-x-1 z-10"}
            />

            <div className="absolute bottom-5 left-5 lg:bottom-9 lg:left-9 z-20">
        ...
        {isTallBlack && (
          <div className="h-full w-full relative">
            <div className="absolute top-3 left-5 lg:top-9 lg:left-9 z-20 max-w-[55%] pointer-events-none">
              ...
              <p className="text-neutral-400 text-xs lg:text-[13.5px] font-semibold mt-2 leading-relaxed">
                {category.tagline}
              </p>
            </div>
  ...
  // DEFAULT VIEW: Exact, original, pixel-perfect layout from your homepage (unaffected)
  if (category.type) {
    ...
    >
      ...
      >
        ...
        <span className="relative mt-auto pt-4 text-[10px] font-bold uppercase tracking-wider text-accent-500/70 transition-colors duration-300 group-hover:text-accent-500">
          {count > 0 ? `${count} parts available` : "Available on inquiry"}
        </span>
      </Link>
    </motion.div>
  );
